...
~some_hook(deployment)~ when making assignments with:

#+BEGIN_SRC matlab
deployment = some_hook(deployment)
#+END_SRC

To avoid this issue, all custom fields added to deployments by hooks should be
added to the ~deployment.cruise~ field.


*** post_setup_hook
~[config] = post_setup_hook(config)~

Modify vessel entries in the main ~Config~ structure before any processing has
...
Modify a ~deployment~'s ~adcp~, and/or ~gps~ data structures after loading
