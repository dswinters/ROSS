...
described below, with some example applications. It can be useful to set up
switch statements within these functions that trigger a certain set of
instructions if a deployment name is matched. These functions allow for the
addition of complex user-defined processing functions without modification of
the core processing code.

*** Additional pre-rotation ADCP processing (~cruise/cruise_proc_pre_rotation.m~)
...
*** Additional post-rotation ADCP processing (~cruise/cruise_proc_post_rotation.m~)

Similar, but after velocities have been rotated to earth coordinates.
